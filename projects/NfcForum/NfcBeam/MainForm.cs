...
namespace SpringCardApplication
{
	public partial class MainForm : Form
	{
		...
		void StartReceiver()
		{
			...
			Trace.WriteLine("Create (and start) SNEP Server (receiver)");
		...
		void StartSender()
		{
			...
			Ndef myNdef = CreateMyNdefMessage();
			...
			{
				Trace.WriteLine("Can't create SNEP Client: nothing to send");
			}
		}
		...
		void MainFormFormClosed(object sender, FormClosedEventArgs e)
		{
			...
			if (rbRecvOnly.Checked) s.SelectedMode = Settings.MODE_RECV_ONLY; else
			if (rbSendOnly.Checked) s.SelectedMode = Settings.MODE_SEND_ONLY; else
			if (rbRecvThenSend.Checked) s.SelectedMode = Settings.MODE_RECV_THEN_SEND; else
			if (rbSendThenRecv.Checked) s.SelectedMode = Settings.MODE_SEND_THEN_RECV; else
		...
		delegate void OnNdefMessageReceivedInvoker(Ndef hisNdef);
		void OnNdefMessageReceived(Ndef hisNdef)
		{
			...
			Trace.WriteLine("*** NDEF message received ***");
			...
			if (hisNdef == null)
			{
				Trace.WriteLine("The message is empty");
				return;
			}
		...
		void OnNdefMessageSent()
		{
			...
			Trace.WriteLine("*** NDEF message sent ***");
