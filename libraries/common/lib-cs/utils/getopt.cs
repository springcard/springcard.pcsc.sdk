					...
					else
					{
						if (this.opterr)
						{
							...
							if (argv[this.optind - 1].StartsWith("--"))
							{
								...
								System.Console.Error.WriteLine("{0}: option ''--{1}'' doesn't allow an argument", msgArgs);
							}
							// +option or -option
							else
							{
								...
								System.Console.Error.WriteLine("{0}: option ''{1}{2}'' doesn't allow an argument", msgArgs);
							}
						}
				...
				else if (pfound.HasArg == Argument.Required)
				{
					...
					else
					{
						if (this.opterr)
						{
							...
							System.Console.Error.WriteLine("{0}: option ''{1}'' requires an argument", msgArgs);
						}
					...
					|| (this.optstring.IndexOf(this.nextchar[0]) == - 1))
				{
					if (this.opterr)
					{
						if (this.argv[this.optind].StartsWith("--"))
						{
							...
							System.Console.Error.WriteLine("{0}: unrecognized option ''--{1}''", msgArgs);
						}
						else
						{
							object[] msgArgs = new object[]{ this.progname,
			...
			if ((temp == null) || (c2 == ':'))
			{
				if (this.opterr)
				{
					if (this.posixlyCorrect)
					{
						...
						System.Console.Error.WriteLine("{0}: illegal option -- {1}", msgArgs);
					}
					else
					{
						object[] msgArgs = new object[]{
			...
			if ((temp[0] == 'W') && (temp.Length > 1) && (temp[1] == ';'))
			{
				...
				else if (this.optind == this.argv.Length)
				{
					if (this.opterr)
					{
						...
						object[] msgArgs = new object[]{
							this.progname, (char) c2 };
						System.Console.Error.WriteLine("{0}: option requires an argument -- {1}", msgArgs);
					}
