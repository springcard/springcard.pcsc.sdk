...
namespace SpringCard.LibCs
{
	...
	public static class setupapi
	{
			...
			try
			{
				...
				for (uint i = 0; ; i++)
				{
					...
					if (Marshal.GetLastWin32Error() == ERROR_NO_MORE_ITEMS)
						CheckError(Translatable.GetTranslation("NoDeviceFoundMatchingFilter", "No device found matching filter."), 0xcffff);
		...
		private static void CheckError(string message, int lasterror = -1)
		{
			...
			{
				if (!stateChanged)
					throw new ApplicationException(String.Format(Translatable.GetTranslation("ErrorDisablingHardwareDevice", "Error disabling hardware device (Code {0}): {1}"), code, message));
				else
					canStop = true;
			}
		}
		...
		private static string GetSerialNumberFromWMI(string searchedPid, string searchedVid)
		{
			if (string.IsNullOrEmpty(searchedVid) || string.IsNullOrEmpty(searchedPid))
				throw new ArgumentException( Translatable.GetTranslation("MissingParameters" ,"All parameters must be filled"));
