...
/* Modifications : */
/* l. 2319 -> 2361 : PaintLineInfo 			*/
/* l. 2406 -> 2413 : PaintP1P2String 		*/
/*	PaintHexAndStringView 							*/
/* l. 2702														 	*/
/* calcul. iHexMaxBytes in function UpdateRectanglePositioning l.2823	*/
/* PAS FINI : paste -> pasteHex	: l.2091 -> 2128	*/
...
namespace Be.Windows.Forms
{
	...
	{
		public bool _useOffsetInsteadOfPage = false;
		public bool _mifare_sector = false;
      ...
      System.Diagnostics.Debug.WriteLine("OnPaint " + DateTime.Now.ToString(), "HexBox");
			...
			if(_lineInfoVisible)
				PaintLineInfo(e.Graphics, _startByte, _endByte, _useOffsetInsteadOfPage);
		...
		void PaintLineInfo(Graphics g, long startByte, long endByte, bool offsetInsteadOfPage)
		{
			...
			/*
			if (_useOffsetInsteadOfPage)
			{
				maxLine = GetGridBytePoint(endByte-(startByte+2)).Y+1; // Add 2 to startByte because the first two bytes are ALWAYS p1 p2	
			} else
			{
				long first_byte_printed = startByte + 2*( startByte / _iHexMaxHBytes);
				endByte = _iHexMaxVBytes*(_iHexMaxHBytes+2) + first_byte_printed+1;
				long intern_endByte = Math.Min(_byteProvider.Length-1, endByte+_iHexMaxHBytes);
				maxLine = (int) ((intern_endByte - first_byte_printed) / (_iHexMaxHBytes+2) + 1);			
			}
			*/
			...
			{
				long firstLineByte;
				if (offsetInsteadOfPage)
				{
					firstLineByte = (startByte + (_iHexMaxHBytes)*i) + _lineInfoOffset;
				} else
				{
					firstLineByte = i + startByte/_iHexMaxHBytes;
				}
				...
				if ( (! _useOffsetInsteadOfPage) && (!_mifare_sector))
				{
					byte p1 = 0;
					byte p2 = 0;			
					if (_byteProviderP1P2 != null)
					{
						if ((firstLineByte*2) < _byteProviderP1P2.Length)
							p1 = _byteProviderP1P2.ReadByte(firstLineByte*2);

						
						if ((firstLineByte*2 + 1) < _byteProviderP1P2.Length)
							p2 = _byteProviderP1P2.ReadByte(firstLineByte*2 + 1);

					}
					
					info = (p1*0x100 + p2).ToString(_hexStringFormat, System.Threading.Thread.CurrentThread.CurrentCulture);
				}
				...
				} else
				{
					...
					if (firstLineByte*(_iHexMaxHBytes+2) < _byteProvider.Length)
						p1 = _byteProvider.ReadByte((_iHexMaxHBytes+2) * firstLineByte);
					
					if ((firstLineByte*(_iHexMaxHBytes+2) + 1) < _byteProvider.Length)
						p2 = _byteProvider.ReadByte((_iHexMaxHBytes+2) * firstLineByte  + 1);
				...
				*/
				...
				if (_mifare_sector)
				{
					int nulls = 2-info.Length;
					if(nulls > -1)
					{
						formattedInfo = new string('0', 2-info.Length) + info;
					}
					else
					{
						formattedInfo = new string('~', 2);
					}					
				...
				PaintP1P2String(g, formattedInfo.Substring(0, 2), brush, new PointF(_recLineInfo.X, bytePointF.Y));
				if (!_mifare_sector)
					PaintP1P2String(g, formattedInfo.Substring(2, 2), brush, new PointF(_recLineInfo.X + 23, bytePointF.Y));
		...
		/*--JIZ--*/
		void PaintP1P2String(Graphics g, string sB, Brush brush, PointF bytePointF)
		{
			g.DrawString(sB.Substring(0,1), Font, brush, bytePointF, _stringFormat);
			bytePointF.X += _charSize.Width;
			g.DrawString(sB.Substring(1,1), Font, brush, bytePointF, _stringFormat);
		...
		void PaintHexAndStringView(Graphics g, long startByte, long endByte)
		{
			...
			int counter = -1;
			//MessageBox.Show("_byteProvider.Length-1=" + (_byteProvider.Length-1).ToString() + " - endByte+_iHexMaxHBytes="+ (endByte+_iHexMaxHBytes).ToString());
			//MessageBox.Show("endByte="+endByte);
			 ...
			 */
				...
				//for (long i = startByte+2; i < intern_endByte+1; i++) /* Add 2 to startByte, because first two bytes are ALWAYS p1 p2	*/
				for (long i = startByte; i < intern_endByte+1; i++)
				{
					counter++;
					Point gridPoint = GetGridBytePoint(counter);
			...
			
			{
				...
				intern_endByte = Math.Min(_byteProvider.Length-1, endByte+_iHexMaxHBytes);

				long i = first_byte_printed + 2;
			
				while(i < intern_endByte+1)
				{
					counter++;
					Point gridPoint = GetGridBytePoint(counter);
					PointF byteStringPointF = GetByteStringPointF(gridPoint);
		...
		void UpdateRectanglePositioning()
		{
			...
			if(_lineInfoVisible)
			{
				if (_mifare_sector)
				{
					_recLineInfo = new Rectangle(_recContent.X+marginLeft,
						_recContent.Y, 
						(int) (_charSize.Width*5), //(int)(_charSize.Width*10),
						_recContent.Height);				
				} else
				{
					_recLineInfo = new Rectangle(_recContent.X+marginLeft,
						_recContent.Y, 
						(int) (_charSize.Width*8), //(int)(_charSize.Width*10),
						_recContent.Height);							
				}
			...
			_recHex = new Rectangle( _recLineInfo.X + _recLineInfo.Width, /*50*/
				_recLineInfo.Y,
		...
		/*------*/
		/// <summary>
		/// Gets or sets the ByteProviderP1P2 : contains values for P1 and P2.
		/// </summary>
		[Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public IByteProvider ByteProviderP1P2
		{
			get { return _byteProviderP1P2; }
			set 
			{ 
				if(_byteProviderP1P2 == value)
					return;
        ...
        IByteProvider _byteProviderP1P2;
