...
namespace SpringCard.NFC
{
	public abstract class LLCP
	{
		...
		protected SCardChannel _channel;
		...
		public LLCP(string ReaderName)
		{
			_channel = new SCardChannel(ReaderName);
		}
		
		public LLCP(SCardReader Reader)
		{
			_channel = new SCardChannel(Reader);
		}
		
		public LLCP(SCardChannel Channel)
		{
			_channel = Channel;
		}
		...
		{
			Trace.WriteLine("Starting clients");
			
			if (_clients != null)
			{
				for (int i=0; i<_clients.Count; i++)
				{
					Trace.WriteLine("Client '" + _clients[i].Name + "' is ready");
		...
		{
			Trace.WriteLine("Link established");
