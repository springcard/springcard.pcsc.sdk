...
namespace SpringCard.NFC
{
	...
	public class LLCP_PDU : CardBuffer
	{
		...
		public override string AsString(string separator)
		{
			...
			{
				if (_bytes.Length > 2)
					s += String.Format("SEQ={0:X02} ", _bytes[2]);
				
				for (i = 3; i < _bytes.Length; i++)
				{
					if (i > 3)
						s = s + separator;
		...
		private void addHeader(byte PTYPE, byte DSAP, byte SSAP)
		{
			...
			_bytes[0] = (byte) (h0 & 0xFF);
			_bytes[1] = (byte) (h1 & 0xFF);
		}
			...
			if (Payload != null)
			{
				if (hasSequence())
				{
					...
					for (int i=0; i<Payload.Length; i++)
						_bytes[3+i] = Payload[i];
				} else
				{
					ensureLength(2 + Payload.Length);
					for (int i=0; i<Payload.Length; i++)
						_bytes[2+i] = Payload[i];
				}
			}
		}
		...
		public byte PTYPE
		{
			get
			{
				...
				r = (byte) (((_bytes[0] << 2) & 0x0C) | ((_bytes[1] >> 6) & 0x03));
		...
		public byte Sequence
		{
			get
			{
				if ((_bytes == null) || (_bytes.Length < 3) || !hasSequence())
					return 0;
		...
		public byte[] Payload
		{
			get
			{
				...
				if (hasSequence())
				{
					...
					byte[] b = new byte[_bytes.Length - 3];
				...
				{
					byte[] b = new byte[_bytes.Length - 2];
					for (int i=0; i<b.Length; i++)
						b[i] = _bytes[i+2];
					return b;
				}
