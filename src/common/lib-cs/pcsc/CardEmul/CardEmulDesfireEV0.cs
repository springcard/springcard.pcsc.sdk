...
#region Internal types
		...
		private abstract class File
		{
		  ...
		  protected bool Base_FromXML(Dictionary<string, string> xml, string type)
		  {
		    ...
		    if (!xml.TryGetValue("Type", out t)) return false;		    
		    if (!t.Equals(type)) return false;
		    if (!xml.TryGetValue("ID", out t)) return false;
		    _fid = byte.Parse(t, System.Globalization.NumberStyles.HexNumber);		    		    
		    if (!xml.TryGetValue("CommSettings", out t)) return false;
		    _comm_settings = byte.Parse(t, System.Globalization.NumberStyles.HexNumber);		    
		    if (!xml.TryGetValue("AccessRights", out t)) return false;
		...
		private abstract class StandardOrBackupFile : File
		{
		  ...
		  {
		    if (!Base_FromXML(xml, type)) return false;
		    
		    string t;
		    if (!xml.TryGetValue("Size", out t)) return false;
		    _size = uint.Parse(t, System.Globalization.NumberStyles.Integer);
		    if (!xml.TryGetValue("Content", out t)) return false;		    
		    _bytes = (new CardBuffer(t)).GetBytes();
		    if ((_bytes == null) || (_bytes.Length != _size)) return false;
		...
		private class StandardFile : StandardOrBackupFile
		{
		  ...
		  public override bool FromXml(Dictionary<string, string> xml)
		  {
		    if (!StandardOrBackup_FromXML(xml, "Standard")) return false;		    
		    return true;
		  }
		...
		private class BackupFile : StandardOrBackupFile
		{
		  ...
		  public override bool FromXml(Dictionary<string, string> xml)
		  {
		    if (!StandardOrBackup_FromXML(xml, "Backup")) return false;		    
		    return true;
		  }
		...
		private abstract class RecordOrCyclicFile : File
		{
		  ...
		  protected bool RecordOrCyclic_FromXML(Dictionary<string, string> xml, string type)
		  {
		    ...
		    string t;
		    if (!xml.TryGetValue("RecordSize", out t)) return false;
		    _record_size = uint.Parse(t, System.Globalization.NumberStyles.Integer);
		    if (!xml.TryGetValue("RecordCount", out t)) return false;
		    _max_record_count = uint.Parse(t, System.Globalization.NumberStyles.Integer);
		    if (!xml.TryGetValue("CurrentRecord", out t)) return false;
		    ...
		    for (int i=0; i<_current_record_count; i++)
		    {
		      ...
		      if (!xml.TryGetValue(n, out t)) return false;
		      byte[] b = (new CardBuffer(t)).GetBytes();
		      if ((b == null) || (b.Length != _record_size)) return false;
      ...
      public RecordFile()
        ...
        _type = 0x03;
		  ...
		  public override bool FromXml(Dictionary<string, string> xml)
		  {
		    if (!RecordOrCyclic_FromXML(xml, "Record")) return false;		    
		    return true;
		  }
	    ...
	    public override void ToXML(XmlTextWriter xtw)
		  ...
		  public override bool FromXml(Dictionary<string, string> xml)
		  {
		    if (!RecordOrCyclic_FromXML(xml, "Cyclic")) return false;		    
		    return true;
		  }
...
#region Glue
      ...
      if ((pending != null) && (pending.Code != 0))
		...
		protected override CardBuffer OnFrame(CardBuffer cmd)
		{
		  CardBuffer cd = null;
		  CardBuffer rd = null;
		  byte cc;
		  ...
		  if (IsFirstCommand)
		  {
		    if ((cmd.Length >= 4) && ((cmd.GetByte(0) == 0x90) || (cmd.GetByte(0) == 0x00)))
        {
		      UseApdus = true;
		      
		      CAPDU capdu = new CAPDU(cmd.GetBytes());
		  ...
		  if (cmd.Length > 1)
		  {		  
		    cd = new CardBuffer(cmd.GetBytes(1, -1));
		  }
		    
		  rs = OnDesfireCommand(cc, cd, ref rd);
		  
		  byte[] rsp;
		  
		  if (rd != null)
		  {
		    rsp = new byte[rd.Length + 1];
		    for (int i=0; i<rd.Length; i++)
		      rsp[i+1] = rd.GetByte(i);
		  } else
		  {
		    rsp = new byte[1];
		  }
		  ...
		  switch (pending.Step)
		  {
		    case 0 :
		      ...
		      for (int i=0; (i<Applications.Count) && (i < 19); i++)
		      {
		        ...
		        Result[3 * i + 2] = (byte) (Aid % 0x00000100); Aid /= 0x00000100;
		        Result[3 * i + 1] = (byte) (Aid % 0x00000100); Aid /= 0x00000100;
    ...
    public bool CheckIn(CardBuffer DataIn, uint Len)
    {
      return CheckIn(DataIn, Len, Len);
		...
		public long Get4(byte[] buf, uint offset)
		{
		  ...
		  t  = buf[offset + 3]; t *= 0x00000100;
		  t += buf[offset + 2]; t *= 0x00000100;
		  t += buf[offset + 1]; t *= 0x00000100;
		  t += buf[offset + 0];
		...
		public uint Get3(byte[] buf, uint offset)
		{
		  ...
		  r  = buf[offset + 2]; r *= 0x00000100;
		  r += buf[offset + 1]; r *= 0x00000100;
		  r += buf[offset + 0];
		...
		public void Set4(ref byte[] buf, uint offset, long val)
		{
		  ...
		  buf[offset + 0] = (byte) (t % 0x00000100); t /= 0x00000100;
		  buf[offset + 1] = (byte) (t % 0x00000100); t /= 0x00000100;
		  buf[offset + 2] = (byte) (t % 0x00000100); t /= 0x00000100;
		...
		{
		  buf[offset + 0] = (byte) (val % 0x00000100); val /= 0x00000100;
		  buf[offset + 1] = (byte) (val % 0x00000100); val /= 0x00000100;
		  buf[offset + 2] = (byte) (val % 0x00000100);
		}
		
		public void Set2(ref byte[] buf, uint offset, ushort val)
		{
		  buf[offset + 0] = (byte) (val % 0x0100); val /= 0x0100;
...
#region XML import/export
    ...
    private bool LoadFromXML_Key(XmlTextReader XTR, ref Key key)
    {
      ...
      while (XTR.Read())
      {
        if (XTR.IsStartElement())
        {
          ...
          {
            Trace.WriteLine("Desfire/Application/Key/" + n);
            return false;
          }
        } else
          break;
    ...
    {     
      ...
      if (file == null)
      {
        Trace.WriteLine("file KO");
        return false;
      }
      
      if (!file.FromXml(xml))
      {
        Trace.WriteLine("FromXml KO");
        return false;
      }
      
      Trace.WriteLine("OK pour ce fichier");
    ...
    {      
      ...
      while (XTR.Read())
      {
        if (XTR.IsStartElement())
        {
          ...
          if (n.Equals("Keys"))
          {
            while (XTR.Read())
            {
              if (XTR.IsStartElement())
              {
                ...
                {
                  Key key = null;
                  if (!LoadFromXML_Key(XTR, ref key)) return false;
          ...
          if (n.Equals("Files"))
          {
            while (XTR.Read())
            {
              if (XTR.IsStartElement())
              {
                ...
                {
                  File file = null;
                  if (!LoadFromXML_Application_File(XTR, ref file)) return false;
          ...
          {
            Trace.WriteLine("Desfire/Application/" + n);
            return false;
          }
        } else
          break;
      ...
      while (xtr.Read())
      {
        if (xtr.IsStartElement())
        {
          ...
          if (inDesfire)
          {
            ...
            if (n.Equals("RootKey"))
            {
              if (!LoadFromXML_Key(xtr, ref RootKey)) goto failed;
              if (RootKey == null) goto failed;
            } else
            if (n.Equals("Applications"))
            {
              while (xtr.Read())
              {
                if (xtr.IsStartElement())
                {
                  if (xtr.Name.Equals("Application"))
                  {
                    ...
                    if (!LoadFromXML_Application(xtr, ref application)) goto failed;
                    if (application == null) goto failed;
